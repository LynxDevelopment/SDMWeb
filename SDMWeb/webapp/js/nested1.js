...
var Nested = new Class({
	...
	initialize: function(list, options) {		
		...
		if (this.options.collapse) {
			...
			this.list.addEvent('dblclick', this.bound.collapse);
		}
	...
	collapse: function(event) {
		var el = $(event.target);
		if(event.type == 'click' && !el.hasClass('treeImage'))
			return true;
		if (this.options.handleClass) {
			while (el.nodeName != this.options.childTag && !el.hasClass(this.options.handleClass) && el != this.list) {
				el = el.getParent();
			...
			if (sub) {
				if (sub.getStyle('display') == 'none') {
					...
					if( el == el.getParent().getLast())
						$E('img[class$=treeImage]',el).setProperty("src",this.options.ncollapseLastImage);
					else
						$E('img[class$=treeImage]',el).setProperty("src",this.options.ncollapseImage);
				} else {
					sub.setStyle('display', 'none');
					el.addClass(this.options.collapseClass);
					if( el == el.getParent().getLast())
						$E('img[class$=treeImage]',el).setProperty("src",this.options.collapseLastImage);
					else
						$E('img[class$=treeImage]',el).setProperty("src",this.options.collapseImage);
				}
			}
		}
		//event.stop();
	...
	organizeTree: function(thisEl) {
		$ES(thisEl.options.parentTag, thisEl.list).each(function(el){
			var sub = $E(thisEl.options.childTag, el);
			if(!sub)
				el.remove();
		});
		$ES('LI', thisEl.list).each(function(el){
			var sub = $E(thisEl.options.parentTag, el);
			if (sub) {
				if (sub.getStyle('display') == 'none') {
					if( el == el.getParent().getLast())
						$E('img[class$=treeImage]',el).setProperty("src",thisEl.options.collapseLastImage);
					else
						$E('img[class$=treeImage]',el).setProperty("src",thisEl.options.collapseImage);
				} else {
					if( el == el.getParent().getLast())
						$E('img[class$=treeImage]',el).setProperty("src",thisEl.options.ncollapseLastImage);
					else
						$E('img[class$=treeImage]',el).setProperty("src",thisEl.options.ncollapseImage);
				}
			}
			else{
				if( el == el.getParent().getLast())
					$E('img[class$=treeImage]',el).setProperty("src",thisEl.options.lastImage);
				else
					$E('img[class$=treeImage]',el).setProperty("src",thisEl.options.insideImage);
			}
		});
	},
	...
	movement: function(event, el) {
		...
		if (over != el && $(over).hasClass(this.options.collapseClass)) {
			check = $E(this.options.parentTag, over);
			over.removeClass(this.options.collapseClass);
			if( over == over.getParent().getLast())
				$E('img[class$=treeImage]',over).setProperty("src",this.options.ncollapseLastImage);
			else
				$E('img[class$=treeImage]',over).setProperty("src",this.options.ncollapseImage);
			if(check)
